...
std::size_t ChunkMerkleHasher::BlockCount(const std::uint64_t bytes) {
	if (bytes == 0) {
		return 0;
	}
...
std::size_t ChunkMerkleHasher::NextPowerOfTwo(const std::size_t value) {
	std::size_t result = 1;
	while (result < value) {
		result *= 2;
	}
...
std::size_t ChunkMerkleHasher::LeafTarget(const std::uint64_t fileBytes, const std::int32_t pieceBytes) {
...
std::vector<libtorrent::sha256_hash> ChunkMerkleHasher::LeafHashes(const std::span<const std::byte> data) {
...
libtorrent::sha256_hash ChunkMerkleHasher::PieceRoot(
	const std::span<const std::byte> pieceData,
	const std::size_t leafTarget
) {
