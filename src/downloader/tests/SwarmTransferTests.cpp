...
ChunkDigest MakeDigest(const char filler) {
...
ModManifest MakeManifest() {
	...
	for (std::size_t index = 0; index < CHUNK_COUNT; ++index) {
		chunks.push_back(ManifestChunk{
				MakeDigest(static_cast<char>('a' + index)), (index * CHUNK_LENGTH), static_cast<std::uint32_t>(CHUNK_LENGTH)
			}
		);
	}
...
TEST_CASE("layout collapses a repeated chunk to one file") {
	...
	std::vector<ManifestChunk> chunks{
		ManifestChunk{repeated, 0, 16}, ManifestChunk{repeated, 16, 16}
	};
