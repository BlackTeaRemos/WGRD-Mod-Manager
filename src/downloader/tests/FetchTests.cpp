...
TEST_CASE("fetch pulls only the chunks that are missing") {
    ...
    const std::vector<std::string> wanted = {
        ChunkFileNaming::FileNameFor(DigestFor(1)),
        ChunkFileNaming::FileNameFor(DigestFor(3))
    };
    ...
    for (std::size_t index = 0; index < lengths.size(); ++index) {
        ...
        payload.read(source.data(), static_cast<std::streamsize>(lengths[index]));

        std::ifstream fetched(chunkFile, std::ios::binary);
        std::vector<char> actual(lengths[index]);
        fetched.read(actual.data(), static_cast<std::streamsize>(lengths[index]));
