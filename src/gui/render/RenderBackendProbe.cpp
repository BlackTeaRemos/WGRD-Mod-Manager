...
std::vector<RenderBackendKind> RenderBackendProbe::PreferredOrder() {
	if (RunningUnderWine()) {
		return {
			RenderBackendKind::Vulkan, RenderBackendKind::OpenGL3, RenderBackendKind::Direct3D11
		};
	}

	return {
		RenderBackendKind::Direct3D11, RenderBackendKind::Direct3D12, RenderBackendKind::Vulkan, RenderBackendKind::OpenGL3
	};
}

std::unique_ptr<IRenderBackend> RenderBackendProbe::Create(const RenderBackendKind kind) {
	switch (kind) {
...
std::unique_ptr<IRenderBackend> RenderBackendProbe::Select(const HWND window) {
