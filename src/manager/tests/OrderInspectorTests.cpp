...
InstallFolder MakeFolder(std::string_view name) {
    const auto folder = InstallFolder::Parse(name);
    REQUIRE(folder.has_value());
    return *folder;
}

InstalledMod MakeMod(std::string_view name, std::vector<std::uint32_t> builds) {
...
TEST_CASE("reports every absent entry") {
    const LoadOrder order(std::vector{
        OrderEntry{MakeFolder("good"), true},
        OrderEntry{MakeFolder("ghost"), true},
        OrderEntry{MakeFolder("vanished"), true}
    });
...
TEST_CASE("emits no other category") {
    const LoadOrder order(std::vector{
        OrderEntry{MakeFolder("good"), true},
        OrderEntry{MakeFolder("ghost"), true},
        OrderEntry{MakeFolder("hidden"), false}
    });
    const std::vector<InstalledMod> installed{MakeMod("good", {131635})};
