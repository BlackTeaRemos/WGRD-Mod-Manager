...
ModManifest MakeManifest(const wgrd::domain::PublisherFingerprint& publisher, std::uint64_t version) {
...
TEST_CASE("receiver accepts a signed announce and rejects a rollback") {
    ...
    SigningKeyStore keyStore(keyPath.Value(), protection);
    ...
    AnnounceReceiver receiver(registry);
...
TEST_CASE("receiver rejects an unregistered and a revoked publisher") {
    ...
    AnnounceReceiver strangerReceiver(empty);
    ...
    AnnounceReceiver revokedReceiver(revoked);
...
TEST_CASE("announce refuses a manifest from another publisher") {
    ...
    REQUIRE(keyStore.Create().has_value());
