...
namespace {
	...
	std::optional<std::string> ReadRegistryString(const HKEY hive, const wchar_t* subKey, const wchar_t* valueName) {
		HKEY key = nullptr;
	...
	std::vector<std::string> ExtractQuotedValues(const std::string& contents, const std::string_view key) {
...
std::optional<domain::GameInstallation> GameLocator::FromRoot(const std::filesystem::path& root) {
	...
	if (!std::filesystem::is_regular_file(root / std::filesystem::path(GAME_EXECUTABLE_NAME), probe) || probe) {
		return std::nullopt;
	}
	...
	if (!std::filesystem::is_directory(modsDirectory, probe)) {
		probe.clear();
		std::filesystem::create_directories(modsDirectory, probe);

		if (probe || !std::filesystem::is_directory(modsDirectory, probe)) {
			return std::nullopt;
		}
	}

	domain::GameInstallation installation{
		root, modsDirectory, modsDirectory / std::filesystem::path(ORDER_FILE_NAME)
	};
...
std::optional<domain::GameInstallation> GameLocator::Resolve() {
	...
	return FromRoot(executable.parent_path());
}
...
std::optional<std::filesystem::path> GameLocator::SteamRoot() {
	return SteamRoot_();
}
