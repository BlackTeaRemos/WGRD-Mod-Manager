...
std::optional<std::uint32_t> ParseComponent(std::string_view text) {
...
VersionNumber::VersionNumber(std::uint32_t major, std::uint32_t minor, std::uint32_t patch) noexcept
    : _major(major),
      _minor(minor),
      _patch(patch) {
}
