...
namespace {
	...
	constexpr int RESOLVE_TIMEOUT = 10000;
	constexpr int CONNECT_TIMEOUT = 15000;
	constexpr int SEND_TIMEOUT = 20000;
	constexpr int RECEIVE_TIMEOUT = 60000;

	class SessionHandle {
	public:
		explicit SessionHandle(const HINTERNET handle) noexcept
			: _handle(handle) {}
...
std::expected<HttpsClient::Target, HttpsError> HttpsClient::CrackUrl_(const std::string_view url) {
	...
	return Target{
		std::wstring(hostBuffer.data()), std::wstring(pathBuffer.data()), (components.nPort)
	};
}

std::expected<unsigned int, HttpsError> HttpsClient::Fetch_(
	const std::string_view url,
	const std::string_view acceptHeader,
...
std::expected<HttpsResponse, HttpsError> HttpsClient::Get(
	...
	const auto status = Fetch_(
		...
		acceptHeader,
		limit,
		[&body](const std::uint8_t* data, const std::size_t length) {
...
std::expected<std::uint64_t, HttpsError> HttpsClient::Download(
	...
	const auto status = Fetch_(
		...
		{},
		limit,
		[&output, &written, &progress](const std::uint8_t* data, const std::size_t length) {
