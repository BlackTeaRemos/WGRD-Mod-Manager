...
std::string GitHubContentsSource::BuildUrl(std::string_view repository, std::string_view folder) {
    return std::format("{}/repos/{}/contents/{}", API_HOST, StripHost(repository), folder);
}

bool GitHubContentsSource::IsRegistryFileName(std::string_view name) {
    ...
    return std::all_of(stem.begin(), stem.end(), [](const char character) {
...
std::expected<std::vector<ContentsEntry>, ContentsLookupError> GitHubContentsSource::ParseListing(
    ...
    for (const Json& element : parsed) {
        ...
        if (!downloadUrl.starts_with("https://")) {
            continue;
        }
