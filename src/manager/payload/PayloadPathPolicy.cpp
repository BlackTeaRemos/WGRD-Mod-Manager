...
namespace {
	...
	bool IsControlCharacter(const char character) {
...
std::string PayloadPathPolicy::ToUpper_(const std::string_view text) {
...
bool PayloadPathPolicy::IsReservedDeviceName_(std::string_view component) {
	...
	return std::ranges::find(RESERVED_NAMES, upper) != RESERVED_NAMES.end();
}

bool PayloadPathPolicy::HasPayloadExtension_(const std::string_view component) {
