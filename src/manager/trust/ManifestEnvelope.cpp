...
std::vector<std::uint8_t> ManifestEnvelope::Encode(
	...
	std::ranges::copy(fingerprintBytes, envelope.begin() + FINGERPRINT_OFFSET);
	...
	std::ranges::copy(signatureBytes, envelope.begin() + SIGNATURE_OFFSET);

	std::ranges::copy(payload, envelope.begin() + HEADER_BYTES);

	return envelope;
}

std::expected<EnvelopeView, domain::ManifestAuthenticationError> ManifestEnvelope::Decode(
	const std::span<const std::uint8_t> envelope
) {
...
std::uint32_t ManifestEnvelope::ReadLittleEndian_(const std::span<const std::uint8_t> source, const std::size_t offset) {
