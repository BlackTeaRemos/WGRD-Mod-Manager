...
std::uint32_t AnnounceCodec::ReadLittleEndian32_(const std::span<const std::uint8_t> source, const std::size_t offset) {
...
std::uint64_t AnnounceCodec::ReadLittleEndian64_(const std::span<const std::uint8_t> source, const std::size_t offset) {
...
std::vector<std::uint8_t> AnnounceCodec::EncodeSignable(const domain::SignedAnnounce& announce) {
	...
	std::ranges::copy(fingerprint, record.begin() + FINGERPRINT_OFFSET);
	...
	std::ranges::copy(manifestDigest, record.begin() + MANIFEST_DIGEST_OFFSET);

	const std::span<const std::uint8_t> torrentInfoHash = announce.torrentInfoHash.Bytes();
	std::ranges::copy(torrentInfoHash, record.begin() + TORRENT_INFO_HASH_OFFSET);
...
std::vector<std::uint8_t> AnnounceCodec::Encode(const domain::SignedAnnounce& announce) {
	...
	std::ranges::copy(signature, record.begin() + SIGNATURE_OFFSET);
...
std::expected<domain::SignedAnnounce, AnnounceDecodeError> AnnounceCodec::Decode(
	...
	return domain::SignedAnnounce{
		*publisher, std::move(modName), ReadLittleEndian64_(record, VERSION_OFFSET), *manifestDigest, *torrentInfoHash, *signature
	};
}
}
