...
UpdateService::UpdateService(std::string repository, VersionNumber current)
    : _client(),
      _source(_client),
...
void UpdateService::Publish_(domain::UpdatePhase phase, std::string message) {
...
void UpdateService::RunCheck_() {
    ...
    if (!release.has_value()) {
        Publish_(domain::UpdatePhase::Failed, "check failed");
        _busy = false;
        return;
    }
