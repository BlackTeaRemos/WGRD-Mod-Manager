...
bool ManifestStore::IsDigestHex_(std::string_view digestHex) {
	...
	return std::ranges::all_of(digestHex, [](const char character) {
...
std::filesystem::path ManifestStore::PathFor_(const std::string_view digestHex) const {
...
std::expected<std::filesystem::path, ManifestStoreError> ManifestStore::Save(
	const std::string_view digestHex,
...
std::filesystem::path ManifestStore::PathFor(const std::string_view digestHex) const {
	return PathFor_(digestHex);
}

bool ManifestStore::Holds(const std::string_view digestHex) const {
	if (!IsDigestHex_(digestHex)) {
		return false;
	}
